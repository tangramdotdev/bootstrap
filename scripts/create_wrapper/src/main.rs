...
struct Args {
	...
	#[clap(
		long,
		help = "Environment to add.  `.` and `..` in the leading position are relative to the output binary"
	)]
	env: Vec<String>,
...
fn main() -> ExitCode {
	...
	let interpreter_link = ctx.interpreter;
	...
	let cc_wrapper_result = std::process::Command::new(&ctx.static_cc)
		...
		.arg(cgen::set_macro(
			...
			cgen::ident("TG_INTERPRETER_FLAVOR_LD_LINUX"),
		))
	...
	if !cc_wrapper_result.success() {
		panic!("failed to compile wrapper binary (exit status {cc_wrapper_result:?}");
	}
